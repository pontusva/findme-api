  ...
  getLostPetReport: (t: ObjectDefinitionBlock<"Query">) =>
    t.nonNull.field("getLostPetReport", {
      ...
      resolve: async (_, { id }) => {
        ...
        if (!report) throw new Error(`Lost pet report with ID ${id} not found`);
        return report;
      },
    }),

  getAllLostPetReports: (t: ObjectDefinitionBlock<"Query">) =>
...
export const lostPetReportMutations = {
  createLostPetReport: (t: ObjectDefinitionBlock<"Mutation">) =>
  ...
  updateLostPetReport: (t: ObjectDefinitionBlock<"Mutation">) =>

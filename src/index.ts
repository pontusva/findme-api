...
const yoga = createYoga<{
  ...
  context: async ({ request }: YogaInitialContext) => {
    ...
    try {
      const session =
        await clerkClient.sessions.verifySession(
          token,
          token
        )
      return {
  ...
  try {
    ...
    fastify.addHook('onRequest', async (request, reply) => {
      reply.header(
        ...
        "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' unpkg.com embeddable-sandbox.cdn.apollographql.com sandbox.embed.apollographql.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: embeddable-sandbox.cdn.apollographql.com sandbox.embed.apollographql.com; connect-src 'self' ws: wss: localhost:* sandbox.embed.apollographql.com; font-src 'self' data:; frame-src sandbox.embed.apollographql.com; frame-ancestors 'self' sandbox.embed.apollographql.com;"
      )
    })
